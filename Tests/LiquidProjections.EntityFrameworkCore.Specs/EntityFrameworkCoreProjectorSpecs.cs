...
{
    namespace EntityFrameworkCoreProjectorSpecs
    {
        public class Given_a_sqlite_projector_with_an_in_memory_event_source :
            GivenSubject<EntityFrameworkCoreProjector<ProductCatalogEntry, string, ProjectorState>>
        {
            protected EventMapBuilder<ProductCatalogEntry, string, EntityFrameworkCoreProjectionContext> Events;
            protected LruProjectionCache<ProductCatalogEntry, string> Cache;
            protected Exception ProjectionException;
            private readonly List<IChildProjector> children = new List<IChildProjector>();
            ...
            public Given_a_sqlite_projector_with_an_in_memory_event_source()
            {
                Given(() =>
                {
                    ...
                    UseThe(new DbContextSetup().Build());
                    UseThe(The<DbContextContainer>().DbContextFactory);

                    Cache = new LruProjectionCache<ProductCatalogEntry, string>(1000, 1.Hours(), 2.Hours(), e => e.Id,
                        () => DateTime.UtcNow);

                    Events = new EventMapBuilder<ProductCatalogEntry, string, EntityFrameworkCoreProjectionContext>();

                    WithSubject(_ =>
                    {
                        return new EntityFrameworkCoreProjector<ProductCatalogEntry, string, ProjectorState>(
                            The<IDbContextFactory>().Create, Events, (entry, id) => entry.Id = id, children)
                        {
                            BatchSize = 10,
                            Cache = Cache,
                        };
            ...
            protected void AddChildProjector(IChildProjector childProjector)
            {
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            public When_a_create_was_requested_but_the_cache_already_contained_that_projection()
            {
                Given(() =>
                {
                    ...
                    using (DbContext dbContext = The<IDbContextFactory>().Create())
                    {
                        dbContext.Add(existingEntry);
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_create_a_new_projection()
            {
                using (DbContext dbContext = The<IDbContextFactory>().Create())
                {
                    ProductCatalogEntry entry = dbContext.Find<ProductCatalogEntry>("c350E");
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            public When_an_idempotent_create_was_requested_on_a_pre_existing_database_projection()
            {
                Given(() =>
                {
                    ...
                    using (DbContext dbContext = The<IDbContextFactory>().Create())
                    {
                        dbContext.Add(new ProductCatalogEntry
                        {
                            Id = "c350E",
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_remove_the_projection()
            {
                using (DbContext dbContext = The<IDbContextFactory>().Create())
                {
                    var entry = dbContext.Find<ProductCatalogEntry>("c350E");
                    entry.Should().BeNull();
                }
            }
        ...
        public class When_an_event_requires_a_custom_action :
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            private EntityFrameworkCoreProjectionContext context;
            ...
            {
                using (var dbContext = The<IDbContextFactory>().Create())
                {
                    var entry = dbContext.Find<ProductCatalogEntry>("c350E");
                    entry.Should().BeNull();
                }
            ...
            {
                context.Should().BeOfType<EntityFrameworkCoreProjectionContext>();
        ...
        public class When_a_custom_state_key_is_set : Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_store_projector_state_with_that_key()
            {
                using (var dbContext = The<IDbContextFactory>().Create())
                {
                    ProjectorState projectorState = dbContext.Find<ProjectorState>("CatalogEntries");
        ...
        public class When_the_projector_state_is_enriched : Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_store_the_custom_property_along_with_the_state()
            {
                using (var dbContext = The<IDbContextFactory>().Create())
                {
                    ProjectorState projectorState = dbContext.Find<ProjectorState>(Subject.StateKey);
                ...
                Given(() =>
                {
                    Events.Map<ProductAddedToCatalogEvent>()
                        .As((productAddedToCatalogEvent, context) =>
                        {
                            using (var dbContext = The<IDbContextFactory>().Create())
                            {
                                projectorStateAfterFirstBatch = dbContext.Find<ProjectorState>(Subject.StateKey);
                            }
                        });
                    ...
                    Events.Map<CategoryDiscontinuedEvent>()
                        .As((productAddedToCatalogEvent, context) =>
                        {
                            using (var dbContext = The<IDbContextFactory>().Create())
                            {
                                projectorStateAfterSecondBatch = dbContext.Find<ProjectorState>(Subject.StateKey);
                            }
                        });
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_rollback_the_transaction()
            {
                using (var dbContext = The<IDbContextFactory>().Create())
                {
                    dbContext.Query<ProductCatalogEntry>().Single().Category.Should().Be("TheCategory");
                }
            }
        }
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_not_handle_any_events_anymore()
            {
                using (var dbContext = The<IDbContextFactory>().Create())
                {
                    dbContext.Query<ProductCatalogEntry>().SingleOrDefault().Should().BeNull();
                }
            }
        }
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public When_there_is_a_child_projector()
            {
                Given(() =>
                {
                    ...
                    {
                        ProductCatalogChildEntry childEntry1 = context.Session.Find<ProductCatalogChildEntry>("c350E");
                        ProductCatalogChildEntry childEntry2 = context.Session.Find<ProductCatalogChildEntry>("c350F");
                    ...
                    var childMapBuilder = new EventMapBuilder<ProductCatalogChildEntry, string, EntityFrameworkCoreProjectionContext>();

                    childMapBuilder.Map<ProductAddedToCatalogEvent>()
                        .AsCreateOf(anEvent => anEvent.ProductKey)
                        .Using((entry, anEvent) => entry.Category = anEvent.Category);

                    AddChildProjector(new ChildProjector<ProductCatalogChildEntry, string>(
            ...
            public void Then_the_parent_projector_should_project_all_the_transactions()
            {
                using (DbContext dbContext = The<IDbContextFactory>().Create())
                {
                    ProductCatalogEntry parentEntry1 = dbContext.Find<ProductCatalogEntry>("c350E");
                    parentEntry1.Should().NotBeNull();
                    parentEntry1.Category.Should().Be("Hybrid");

                    ProductCatalogEntry parentEntry2 = dbContext.Find<ProductCatalogEntry>("c350F");
            ...
            public void Then_the_child_projector_should_project_all_the_transactions()
            {
                using (DbContext dbContext = The<IDbContextFactory>().Create())
                {
                    ProductCatalogChildEntry childEntry1 = dbContext.Find<ProductCatalogChildEntry>("c350E");
                    childEntry1.Should().NotBeNull();
                    childEntry1.Category.Should().Be("Hybrid");

                    ProductCatalogChildEntry childEntry2 = dbContext.Find<ProductCatalogChildEntry>("c350F");
            ...
            Given_a_sqlite_projector_with_an_in_memory_event_source
        {
            ...
            public void Then_it_should_rollback_the_transaction()
            {
                using (var dbContext = The<IDbContextFactory>().Create())
                {
                    dbContext.Set<ProductCatalogEntry>().Should().BeEmpty();
                }
            }
        }
        ...
        internal class ProductCatalogEntryConfiguration : IEntityTypeConfiguration<ProductCatalogEntry>
        {
            public void Configure(EntityTypeBuilder<ProductCatalogEntry> builder)
            {
                ...
                builder.Property(x => x.AddedBy).HasMaxLength(100);
        ...
        internal class ProductCatalogChildEntryConfiguration : IEntityTypeConfiguration<ProductCatalogChildEntry>
        {
            public void Configure(EntityTypeBuilder<ProductCatalogChildEntry> builder)
            {
                builder.HasKey(x => x.Id);
                builder.Property(x => x.Id).IsRequired().HasMaxLength(100);
                builder.Property(x => x.Category).HasMaxLength(100);
