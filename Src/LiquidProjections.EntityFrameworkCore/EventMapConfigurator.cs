...
namespace LiquidProjections.EntityFrameworkCore
{
        ...
        where TProjection : class, new()
    {
        ...
        {
            TProjection projection = await cache.Get(key, () => Task.FromResult(context.Session.Find<TProjection>(key)));
            if ((projection == null) || shouldOverwrite(projection))
            {
                if (projection == null)
                {
                    ...
                    context.Session.Add(projection);
                ...
                else
                {
                    ...
                    context.Session.Attach(projection);
                    await projector(projection).ConfigureAwait(false);
                }
            }
        }
        ...
        private async Task<bool> OnDelete(TKey key, EntityFrameworkCoreProjectionContext context)
        {
            TProjection existingProjection = 
                await cache.Get(key, () => Task.FromResult(context.Session.Find<TProjection>(key)));

            if (existingProjection != null)
            {
                context.Session.Remove(existingProjection);
                cache.Remove(key);
